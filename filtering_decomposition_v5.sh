#!/bin/bash
# =====================================================================
#  filtering_decomposition_v5.sh
#  BLuSH-style fMRI preprocessing + decomposition + HTML summary
#  Added: Detrending + improved PLS using binary regressor
# =====================================================================
...
if method=="pca":
    ...
    # Binary regressor: 0 = baseline, 1 = signal
...
echo -e "${DARKRED}========== filtering_decomposition_v5 ==========${RESET}"
echo "TR,$TR," >> "$LOG"
...
# --- STEP 2.5: Detrending ---
echo -e "${DARKRED}STEP 2.5: DETRENDING${RESET}"
read -p "Apply detrending (remove scanner drift)? (y/n): " DETR
if [[ "$DETR" == "y" ]]; then
  3dDetrend -polort 3 -prefix detrended_${TMP} ${TMP}
  TMP="detrended_${TMP}"
  echo "Detrending,Applied,polort3" >> "$LOG"
else
  echo "Detrending,Skipped," >> "$LOG"
fi
...
while true; do
  ...
  if [[ "$METHOD" == "pls" ]]; then
    read -p "Baseline start/end for regressor (e.g. 1000 1050): " DB1 DB2
    read -p "Signal start/end for regressor (e.g. 1500 2000): " DS1 DS2
  else
    DB1=0; DB2=0; DS1=0; DS2=0
  fi
  ...
  if [[ -f "$DEN" ]]; then
    ...
    read -p "Baseline (e.g. 1000 1050): " XB1 XB2; read -p "Signal (e.g. 1500 2000): " XS1 XS2
    OUTD="soner_pipeline/scm_outputs/${SUB}_${METHOD}_B${XB1}-${XB2}_S${XS1}-${XS2}"
    scm_blush_fast "$DEN" "$XB1" "$XB2" "$XS1" "$XS2" "$MASK" "$OUTD"
    fsleyes "$OUTD/norm_func_${XB1}_${XB2}.nii.gz" "$OUTD/baseline_${XB1}_${XB2}.nii.gz" "$OUTD/signal_change_map_${XB1}_${XB2}_${XS1}_${XS2}.nii.gz" &
  fi
done
